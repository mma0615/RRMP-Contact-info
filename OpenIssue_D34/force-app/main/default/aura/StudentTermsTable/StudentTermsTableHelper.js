({
    ...
    //Second, we check to see if they are within the date range for accessing the editor
    ...
    //Third, we parse through all the picklist options and save in an object. TODO: don't rely on indexes
        ...
        action.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS") {
                ...
                for (var r = 0; r < rows.length; r++) {
                    ...
                    if (row.Application__r) {
                        row.ApplicationType = row.Application__r.Application_Type__c;
                        row.FederalFAEligible = row.Application__r.Federal_FA_Eligible__c;
                    }
                }
    ...
    reformatDate: function format(inputDate) {
        var date = new Date(inputDate.replace(/-/g, '\/').replace(/T.+/, ''));
    ...
    jQueryDataTable: function(component) {
        setTimeout(function() {
            ...
            function populatePicklist(fieldName) {
                ...
                switch (fieldName) {
                    case 'FA_File_Completion_Deadline__c':
                        var options = picklists[1].types;
                        break;
                    case "Applied_for_ASSET__c":
                        var options = picklists[0].types;
                        break;
                    case "Asset_Application_Approved__c":
                        var options = picklists[2].types;
                        break;
                    case "Enrollment_Type__c":
                        var options = picklists[3].types;
                        break;
                    case "Has_Student_Completed_4_Year_Degree__c":
                        var options = picklists[4].types;
                        break;
                    case "Program_Enrolled__c":
                        var options = picklists[5].types;
                        break;
                    case "SAP_Status__c":
                        var options = picklists[6].types;
                        break;
                    case "Financial_Aid_Status__c":
                        var options = picklists[7].types;
                        break;
                    case "Enrollment_Status__c":
                        var options = picklists[8].types;
                        break;
                }
                return options;
            }
    ...
    convertArrayOfObjectsToCSV : function(component,objectRecords){
        ...
        //Conditional to render Enrollment status or enrollment type, depending on 
        //whether the user is associated with Emily Griffith Technical College
        var userAccount = component.get("v.userAccount");
        if (userAccount == '001j000000W7QYuAAN') {
            var enrollmentField = 'Enrollment_Status__c',
                enrollmentName = 'Enrollment Status';
        } else {
            var enrollmentField = 'Enrollment_Type__c',
                enrollmentName = 'Enrollment Type';
        }
        ...
        var columnLabels = [
            ...
            'Program Enrolled',
            'Federal FA Eligible',
            'DSF Deadline for FA Requirements',
            'Did Student Complete FA Reqs by DSF Deadline?',
            'Current Financial Aid Status',
            'Final EFC Upon FA File Completion',
            'Did Student Complete College\'s Process to be Considered for In-State Tuition under ASSET (if applicable)',
            'Did Student Qualify for Resident Tuition under ASSET (if applicable)',
            'SAP Status',
